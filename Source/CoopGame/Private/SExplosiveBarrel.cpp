...
ASExplosiveBarrel::ASExplosiveBarrel()
{
	...
	//Set to physics body to let radial component affect us (ie. when a nearby barrel explodes)
...
void ASExplosiveBarrel::OnHealthChanged(USHealthComponent* OwningHealthComp, float Health, float HealthDelta, const UDamageType* DamageType, AController* InstigatedBy, AActor* DamageCauser)
{
	...
	if (Health <= 0.f)
	{
		...
		Barrels.Add(this);
		//Apply radial damage
		UGameplayStatics::ApplyRadialDamage(GetWorld(), BaseDamage, GetActorLocation(), ExplosionRadius, 0, Barrels, this, GetInstigatorController(), true);
	}
